...
{
	cout << "CMyString 기본 생성자가 실행되었습니다" << endl;
...
{
	cout << "CMyString 소멸자가 실행되었습니다" << endl;
	//객체 소멸전에 메모리해제
	Release();
}
...
{
	if (m_pszData != NULL && rhs.m_pszData != NULL) //유효성 검사
	{
		if (strcmp(m_pszData, rhs.m_pszData) == 0) //문자열이 동일한지 비교, strcmp함수는 같으면0, 다르면 0이 아닌값 반환
		{
			return 1;	//같으면 1 반환
		}
		else
...
int CMyString:: operator !=(const CMyString& rhs)
{
	if (m_pszData != NULL && rhs.m_pszData != NULL) //유효성 검사
	{
		...
		{
			return 0;	//문자열이 같으면 0 반환  **strcmp와 반환과는 다름**
		}
		else
			return 1; // 같지 않으면 1반환
	}
